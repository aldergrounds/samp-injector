...
- **Заголовки для сообщений об ошибках (`Error message titles`)**
   - `ERROR_TITLE_SAMP`: `const wchar_t*`, определяющий заголовок по умолчанию для диалоговых окон ошибок, связанных со сбоями SA-MP (значение: `L"SA-MP Injector Error"`).
   - `ERROR_TITLE_OMP`: `const wchar_t*`, определяющий заголовок по умолчанию для диалоговых окон ошибок, связанных со сбоями OMP (значение: `L"OMP Injector Error"`).
